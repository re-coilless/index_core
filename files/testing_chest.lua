return function( gui, uid, pic_x, pic_y, inv_data, data, zs, xys, slot_func )
    ...
    if( #this_data > 0 ) then
        ...
        --open only if the full inv is active + the full inv mode is engaged, else show the indicator that is lootable
        ...
        for i,line in ipairs( inv_data ) do
            for e,slot in ipairs( line ) do
                uid, w, h = slot_setup( gui, uid, pic_x, pic_y, zs, data, this_data, slot_func, slot, w, h, "universal", inv_id, {i,-e}, false )
