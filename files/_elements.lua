...
function new_generic_hp( gui, uid, screen_w, screen_h, data, zs, xys )
    ...
    if( #this_data > 0 and ComponentGetIsEnabled( this_data[1])) then
        ...
        if( max_hp > 0 ) then
            ...
            uid = new_font_vanilla_small( gui, uid, pic_x + 13, pic_y, zs.main, get_short_num( math.min( math.floor( hp*25 + 0.5 )), 9e99 ), { 255, 255, 255, 0.9 })
            ...
            if( this_data[5] < 31 ) then
                ...
                uid = new_image( gui, uid, pic_x - length, pic_y + 1, zs.main + 0.001, "data/ui_gfx/hud/colors_health_bar_damage.png", 0.5*length*hp/max_hp, 2, ( 30 - this_data[5])/30 )
            end
...
function new_generic_mana( gui, uid, screen_w, screen_h, data, zs, xys )
    ...
    if( #this_data > 0 ) then
        ...
        if( this_data[2]) then
            ...
            if( #this_data > 0 ) then
                local suck_comp = EntityGetFirstComponentIncludingDisabled( data.active_item, "MaterialSuckerComponent" )
                value = suck_comp == nil and this_data[2] or ComponentGetValue2( suck_comp, "barrel_size" )
                if( value >= 0 ) then
                    value = math.min( math.max( this_data[3], 0 ), value )/value
        ...
        if( value >= 0 ) then
            ...
            if( potion_data[3] ~= nil ) then
                uid = new_image( gui, uid, pic_x - 40, pic_y + 1, zs.main + 0.001, potion_data[2], 40*value, 2 )
                colourer( gui, potion_data[3])
            end
            uid = new_vanilla_bar( gui, uid, pic_x, pic_y, {zs.main_back,zs.main}, {40,2,40*value}, potion_data[2] or "data/ui_gfx/hud/colors_mana_bar.png", throw_it_back, potion_data[4])
        end
...
function new_generic_gold( gui, uid, screen_w, screen_h, data, zs, xys )
    ...
    if( #this_data > 0 and this_data[3] >= 0 ) then
        ...
        if( not( this_data[2])) then
            ...
            god_i_love_money_holy_fuck = get_short_num( data.memo.money )
        end

        uid = new_image( gui, uid, pic_x + 2.5, pic_y - 1.5, zs.main, "data/ui_gfx/hud/money.png" )
        uid = new_font_vanilla_small( gui, uid, pic_x + 13, pic_y, zs.main, god_i_love_money_holy_fuck, { 255, 255, 255, 0.9 })
...
function new_generic_orbs( gui, uid, screen_w, screen_h, data, zs, xys )
    ...
    if( data.orbs > 0 ) then
        ...
        uid = new_font_vanilla_small( gui, uid, pic_x + 13, pic_y, zs.main, data.orbs, { 255, 255, 255, 0.9 })
        pic_y = pic_y + 8
    end
